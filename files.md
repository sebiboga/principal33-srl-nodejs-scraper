...
| `src/job-validator.js` | Shared validation primitives - exports validateByHead(url), validateByContent(url, opts), DEFAULT_EXPIRED_KEYWORDS. Used by both `validate-jobs.js` and `tests/validate-principal33-jobs.js`. |
...
| `tests/company.json` | Mock ANAF company data for Principal33 used in unit tests |
| `tests/validate-principal33-jobs.js` | **Principal33 fast validator (used by CI).** HEAD requests only. Called nightly by `automation-testing.yml`. Supports `--dry-run` and `--delete`. |
...
| `tests/e2e/scraper.test.js` | E2E tests - full pipeline with real Personio API, ANAF, and SOLR |
...
| `ROBOTS.md` | robots.txt analysis and scraping policy for Principal33 |
...
| `delete_request.json` | **Manual maintenance tool** — SOLR payload to delete ALL jobs for CIF 42574513. Use only when you need to wipe Principal33 jobs from SOLR entirely. Run with: `curl --user "${SOLR_AUTH}" "https://solr.peviitor.ro/solr/job/update?commit=true" -H "Content-Type: application/json" -d @delete_request.json` |
...
- Full workflow: validate company (ANAF+Peviitor) → scrape Personio → transform → upsert SOLR → generate docs/jobs.md
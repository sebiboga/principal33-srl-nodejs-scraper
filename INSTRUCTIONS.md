...
This scraper extracts job listings from Principal33 Personio careers page and imports them to peviitor.ro.

Target: https://principal33.jobs.personio.de/search.json
...
1. **Start with brand** - We know the brand (e.g., "Principal33")
...
8. **Scrape new jobs** - Extract jobs from Principal33 Personio careers page
...
> **Important**: Scraper does NOT delete jobs from other sources (ANOFM, etc). It only upserts Principal33 jobs. Existing jobs are preserved.
...
3. **Scrape jobs** - Extract jobs from Principal33 Personio API
...
company.js (validate company)
    ...
    │
    ▼ (if active)
scrape Personio API (jobs from Principal33)
    │
    ▼
...
| `tests/validate-principal33-jobs.js` | CI fast validator (HEAD only); thin CLI over `src/job-validator.js` + `solr.js` |
...
| `tests/e2e/scraper.test.js` | End-to-end tests with real Personio API |

...
const PRINCIPAL33_ANAF_RECORD = {
  cui: 42574513,
  name: 'PRINCIPAL33 S.R.L.',
  address: 'GĂRII, 13A, Municipiul Braşov, Braşov',
  caenCode: '6210',
  inactive: false,
  vatRegistered: true,
  eFacturaRegistered: true,
  headquartersAddress: { locality: 'Municipiul Braşov' }
};
...
describe('company.js', () => {
  ...
  describe('getCompanyData (no cache)', () => {
    it('should fetch PRINCIPAL33 via direct CIF lookup and return company data', async () => {
      mockFetch.mockResolvedValueOnce(anafCompanyResponse(PRINCIPAL33_ANAF_RECORD));

      const result = await company.getCompanyData();

      expect(result).toHaveProperty('company', 'PRINCIPAL33 S.R.L.');
      expect(result).toHaveProperty('cif', '42574513');
      expect(result).toHaveProperty('active', true);
      expect(result).toHaveProperty('anafData');
      expect(result.anafData.name).toBe('PRINCIPAL33 S.R.L.');
    });
    ...
    it('should throw when ANAF returns no company name', async () => {
      mockFetch.mockResolvedValueOnce(anafCompanyResponse({ cui: 42574513, name: null }));
  ...
  describe('getCompanyData (with cache)', () => {
    ...
    it('should use cached company data when available', async () => {
      ...
      expect(result.cif).toBe('42574513');
  ...
  describe('validateAndGetCompany', () => {
    ...
    it('should return company data with status active', async () => {
      mockFetch
        .mockResolvedValueOnce(anafCompanyResponse(PRINCIPAL33_ANAF_RECORD))
        .mockResolvedValueOnce(solrResponse(5, [
          { url: 'https://test.com/1', title: 'Job 1' },
          { url: 'https://test.com/2', title: 'Job 2' }
        ]))
        .mockResolvedValueOnce(peviitorResponse([{ company: 'PRINCIPAL33 S.R.L.' }]));
    ...
    if (PRINCIPAL33_ANAF_RECORD.inactive) {
      it('should return inactive status when company is inactive', async () => {
        const inactiveRecord = { ...PRINCIPAL33_ANAF_RECORD, inactive: true };

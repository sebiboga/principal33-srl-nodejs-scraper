...
const ANRAF_RECORD = {
  ...
  registrationNumber: 'J2021002455083',
  vatRegistered: true,
...
const CACHED_DATA = {
  ...
  inactive: false,
  onrcStatusLabel: 'Funcțiune',
  administrators: [{ name: 'OCHIANĂ BOGDAN', role: 'administrator' }],
  authorizedCaenCodes: ['5821', '5829', '6210', '6220', '6290', '6310', '6391', '6392', '6820', '7020', '7414', '8299']
};
...
describe('src/anaf.js', () => {
  ...
  describe('searchCompany', () => {
    it('should return array of companies for valid brand', async () => {
      mockFetch.mockResolvedValue(anafSearchResponse([
        { cui: 42574513, name: 'PRINCIPAL33 S.R.L.', statusLabel: 'Funcțiune' }
      ]));
    ...
    it('should throw on HTTP error', async () => {
      ...
      await expect(anaf.searchCompany('Principal33')).rejects.toThrow('ANAF search error: 500');
    });
    ...
    it('should encode brand name in URL', async () => {
      ...
      await anaf.searchCompany('PRINCIPAL33 SRL');
      expect(capturedUrl).toContain(encodeURIComponent('PRINCIPAL33 SRL'));
    });
  });
  ...
  describe('getCompanyFromANAFWithFallback', () => {
    it('should return fresh data when API works', async () => {
      ...
      const data = await anaf.getCompanyFromANAFWithFallback('42574513');
    ...
    it('should use cached data when API fails', async () => {
      ...
      const data = await anaf.getCompanyFromANAFWithFallback('42574513', CACHED_DATA);
    ...
    it('should throw when API fails and no cache available', async () => {
      ...
      await expect(anaf.getCompanyFromANAFWithFallback('42574513')).rejects.toThrow();
    });
  });
});
